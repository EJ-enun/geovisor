...
def find_center(coord):
    lat = []
    lng = []
    lat = [x for x in coord['lat']]
    lng = [x for x in coord['lng']]

    return (sum(lat)/len(lat)), (sum(lng)/len(lng))

def haversine(central_coord, list_coord):
    import math

    # Coordinates in decimal degrees (e.g. 2.89078, 12.79797)
    clat, clng = central_coord
    lat = [x for x in list_coord['lat']]
    lng = [x for x in list_coord['lng']]
    ...
    b = [math.sin(x/2.0)**2 + math.cos(y) * math.cos(clat) * math.sin(z/2.0)**2 for x,y,z in zip(delta_ph, lat, delta_lamb)]
...
app.layout = dbc.Container(fluid=True, children=[
    ...
    dbc.Row([
        ...
        ### plots
        dbc.Col(children=[
            dbc.Card(dbc.Col(children=[dcc.Graph(id='map'),html.Br(), html.H1(id='output-panel')]), style={'height':'100vh'}),
            ])
        ])
    ])
    ...
    map_figure = {
        'data': [
            go.Scattermapbox(
                lat=tmp['lat'],
                lon=tmp['lng'], 
                text=np.stack((tmp['city'],tmp['district'],tmp['postal'], tmp['dist'], tmp['meters']),axis=-1),
                mode='markers',
...
def render_output_panel(n_clicks, city, industry, cluster_size):
    ...
    panel = html.Div([
        ...
        dbc.Card(body=True, className="text-white bg-primary", children=[
            ...
            html.H6("Districts:", style={"color":"white"}),
            html.H3("{}".format(tmp['district'].to_string()), style={"color":"white"}),

            html.H6("Population:", style={"color":"white"}),
            html.H3("{}".format(pop.to_string()), style={"color":"white"}),
